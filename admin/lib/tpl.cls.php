...
class tpl
{
	...
	public function fetch($file,$par='',$type = 0,$cachename = NULL)
    {
    	...
    	$this->initFile();//就是创建data/complile/content/tpls/app/下的文件和data/html/content/tpls/app/下的文件

    	$source = 'app/'.$this->dir.$file.'.tpl';//app/content/tpls/app/index.tpl
    	$outfile = 'data/compile/'.$this->dir.'%%cpl%%'.$file.'.php';

		if($cachename)$outcache = 'data/html/'.$this->dir.$cachename.'.html';
		else
    	$outcache = 'data/html/'.$this->dir.$file.$par.'.html';//data/html/content/tpls/app/index.html
