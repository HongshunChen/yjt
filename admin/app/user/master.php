...
class app
{
	...
	public function ceshi()
	{
     ...
     if ($phone !=''&&$getip !='') {
     	$args =  $this->sms->getSmsjudge($phone,$getip);
     }else{
	...
	public function index()
	{
			...
			header("Location:index.php?user-master-user");
			exit();
		//$this->sms->getSmsjudge('15665772270','127.0.0.1');
		$subaction = $this->ev->url(3);
		switch($subaction)
		{
			default:
		
		// exit(header("Location:index.php?user-master-user"));
	...
	public function user()
	{
		...
		switch($subaction)
		{
			...
			$event= array(
				'event'=>'删除用户操作',
				'content'=>'删除了用户账号为'.$user['username'].'用户！',
				);
			...
			if($this->ev->post('insertUser'))
			{
				...
				{
					$userphone = $this->user->getUserByEmail($args['userphone']);
					if ($userphone)
					$errmsg = "这个手机号已经被注册";
					if($errmsg)
					{
			...
			if($search['stime'] || $search['etime'])
			{
				...
				if($search['etime']){
					...
					$args[] = array('AND',"userregtime <= :usertime",'usertime',$etime);
				}
			}
			
			$args[]=array('AND','usergroupid = :usergroupid','usergroupid',8);
		...
		switch ($action) {
			...
			if($this->ev->post('insertUser'))
			{
				...
				if($args['userpassword'] == $args['userpassword2'])
				{
					...
					$event = array(
						'event'=>'增加管理员操作',
						'content'=>'增加了管理员账号为'.$args['username'].'后台用户',
						);
					$this->journal->addJournal($event);
					$message = array(
						'statusCode' => 200,
						"message" => "操作成功",
					    "callbackType" => "forward",
					    "forwardUrl" => "index.php?user-master-adminuser&page={$page}{$u}"
					);
					exit(json_encode($message));
				}
			}
			...
			$page = $this->ev->get('page');
			$page = $page>1?$page:1;
			$args = array(array('AND',"usergroupid=:usergroupid","usergroupid",1));
			$pages = $this->user->getUserList($page,20,$args);
			$this->tpl->assign('users',$pages);
			$this->tpl->display('admin_index');
				break;
		}
	}
