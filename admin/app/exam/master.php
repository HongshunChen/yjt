...
class app
{
    ...
    {
        exit(header("Location:index.php?exam-master-questions"));
        ...
        switch ($subaction) {
            ...
            case 'modifyquestion':
                if ($this->ev->get('modifyquestion')) {
                    ...
                    $questionarea = $this->exam->getQuestionarea(array(array("AND", "areaid = :areaid", 'areaid', $question['questionsarea'])), false);
                    $subjects = $this->basic->getSubjectList();
            ...
            default:
                ...
                if ($search['questype']) {
                    $args[] = array("AND", "questype = :questype", 'questype', $search['questype']);
                }

                if ($search['question']) {
                    $args[] = array("AND", "question like :question", 'question', $search['question'], 'like');
                }
                ...
                if ($search['stime'] || $search['etime']) {
                    ...
                    if ($search['stime']) {
                        ...
                        $args[] = array('AND', "questioncreatetime >= :stime", 'stime', $stime);
                    }
                    if ($search['etime']) {
                        $etime = strtotime($search['etime']);
                        $args[] = array('AND', "questioncreatetime <= :etime", 'etime', $etime);
                    }
                }
                if ($search['questionusername']) {
                    $args[] = array("AND", "questionusername = :questionusername", 'questionusername', $search['questionusername']);
                }
                ...
                $subjects = $this->basic->getSubjectList();
                $sections = $this->section->getSectionListByArgs(array("AND", "sectionsubjectid = :sectionsubjectid", 'sectionsubjectid', $search['questionsubjectid']));
                $knows = $this->section->getKnowsListByArgs(array(array("AND", "knowsstatus = 1"), array("AND", "knowssectionid = :knowssectionid", 'knowssectionid', $search['questionsectionid'])));
                $questionarea = $this->exam->getQuestionarea(array(array()), false);
    ...
    public function recyle()
    {
        ...
        switch ($subaction) {
            ...
            default:
                $page = $this->ev->get('page');
                $page = $page > 0 ? $page : 1;
                $args = array(array("AND", "questionstatus = '0'"), array("AND", "questionparent = 0"));
                $questypes = $this->basic->getQuestypeList();
            ...
            case 'clearhistory':
                if ($search['argsmodel']) {
                    if ($search['stime'] || $search['etime']) {
                        ...
                        if ($search['etime']) $args[] = array("AND", "ehstarttime <= :ehstarttime", 'ehstarttime', strtotime($search['etime']));
            ...
            case 'batopen':
                if ($this->ev->get('batopen')) {
                    ...
                    if ($userids && $basics && $days) {
                        ...
                        $basics = explode(",", $basics);
                        $userids = $this->user->getUsersByArgs(array(array("AND", "find_in_set(username,:username)", 'username', $usernames)));
                        ...
                        $basics = explode(",", $basics);
                        $userids = $this->user->getUsersByArgs(array(array("AND", "find_in_set(usergroupid,:usergroupid)", 'usergroupid', $usergroupids)));
                        foreach ($userids as $user) {
                            foreach ($basics as $basicid) {
                                $this->basic->openBasic(array('obuserid' => $user['userid'], 'obbasicid' => $basicid, 'obendtime' => TIME + $days * 24 * 3600));
                            }
