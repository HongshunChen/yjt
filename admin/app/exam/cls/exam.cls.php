...
class exam_exam
{
	...
	{
		$sessionid = $this->session->getSessionId();
		$data = array('examsession',$args,array(array("AND","examsessionid = :examsessionid",'examsessionid',$sessionid)));
...
/**
 * 查询地区
 * @param  [type]  $args       要查询的条件
 * @param  [type]  $bloo       是否要查询全表
 * @return [type]              查询后的数据反馈
 */

	public function getQuestionarea($args,$bloo)
	{

		if ($bloo) {
			$data = array(false,'area',$args);
		}else{
			$data = array(false,'area');
		}
	...
	public function getQuestionsList($page,$number = 20,$args = 1)
	{
		$page = $page > 0?$page:1;
		$r = array();
		$data = array('DISTINCT *','questions',$args,false,'questionid DESC',array(intval($page-1)*$number,$number));
		$sql = $this->pdosql->makeSelect($data);
		
		$r['data'] = $this->db->fetchAll($sql,false,array('questionhtml','questionknowsid'));
		$data = array('count(DISTINCT questionid) AS number','questions',$args);
		$sql = $this->pdosql->makeSelect($data);
