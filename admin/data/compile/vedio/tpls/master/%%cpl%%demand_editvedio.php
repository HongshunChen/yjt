...
<?php } ?>
	      ...
	      <form action="index.php?vedio-master-demand-editvedio" method="post" class="form-horizontal">
				...
				<div class="control-group">
					...
					<div class="controls">
						<input id="basic" name="args[videoname]" type="text" value="<?php echo $this->tpl_var['cat']['videoname']; ?>" needle="needle" msg="您必须输入视频名称" />
					</div>
				</div>
    ...
    var upload = new PolyvUpload(obj);
				...
				<div class="control-group">
					...
					<div class="controls">
						...
						<?php $vid = 0;
 foreach($this->tpl_var['kecheng'] as $key => $val){ 
 $vid++; ?>
						<?php if($val['courseid'] == $this->tpl_var['cat']['courceid']){ ?>
		        		<option value="<?php echo $val['courseid']; ?>"><?php echo $val['coursename']; ?></option>
		        		<?php } ?>
		        		<?php } ?>
				  		<?php $cid = 0;
 foreach($this->tpl_var['kecheng'] as $key => $cat){ 
 $cid++; ?>
				  		<?php if($cat['courseid'] != $this->tpl_var['cat']['courceid']){ ?>
				  		<option value="<?php echo $cat['courseid']; ?>"><?php echo $cat['coursename']; ?></option>
				  		<?php } ?>
				  		<?php } ?>
				  		</select>
					</div>
...
<embed width="320" height="240" src="<?php echo $this->tpl_var['cat']['remoteurl']; ?>" id="VEDIO_IMG" />
					</div>
					<div id="HTML">
				...
				<div class="control-group">
					...
					<div class="controls">
						<textarea class="input-xlarge" rows="4" name="args[content]" id="basicprice"><?php echo $this->tpl_var['cat']['content']; ?></textarea>
				...
				<div class="control-group">
					<div class="controls">
						...
						<input type="hidden" name="args[remoteurl]" id="remoteurl" value="<?php echo $this->tpl_var['cat']['remoteurl']; ?>">
						<input type="hidden" name="args[duration]" value="<?php echo $this->tpl_var['cat']['duration']; ?>" id="duration"> 
						...
						<input type="hidden" name="args[videovid]" id="videovid" value="<?php echo $this->tpl_var['cat']['videovid']; ?>">
						<input type="hidden" name="args[mp4url]" id="mp4" value="">
						<input type="hidden" name="catid" value="<?php echo $this->tpl_var['cat']['videoid']; ?>">
