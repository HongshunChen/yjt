var jingdianshoucang = {
	...
	dataInit: function() {
		...
		jingdianshoucang.fn.initialize();
		//		jingdianshoucang.fn.list_init();
	},
	eventBind: function() {},
	fn: {
		list_init: function() {},
		...
		initialize: function() {
			jingdianshoucang.fn.tagFill(); 
		},
		cordavaDataGet: function() {
			jingdianshoucang.dataInit();
			jingdianshoucang.eventBind();
			...
			if (!config.validation.mobile(number)) {
				...
				$.ajax({
					...
					success: function(list) {
						...
						if (list.status == 1) {
							//登录成功操作
							jingdianshoucang.fn.dataFill(list);
						} else {
							window.location.href="../login_register/deng.html";
							//$.alert(list.data);
						}
...
$(function($) {
	jingdianshoucang.fn.cordavaDataGet();
});